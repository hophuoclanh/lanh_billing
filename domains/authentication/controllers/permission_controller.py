...
def get_all_permissions(
    ...
    if not current_user.has_permission('read', 'permission'):  # Modified the permission argument
...
def create_permission(
    ...
    if not current_user.has_permission('create', 'permission'):  # Modified the permission argument
        raise HTTPException(status_code=403, detail='Not authorized')
...
def update_permission(
    ...
    if not current_user.has_permission('update', 'permission'):  # Modified the permission argument
        raise HTTPException(status_code=403, detail='Not authorized')
...
def delete_permission(
    ...
    if not current_user.has_permission('delete', 'permission'):  # Modified the permission argument
        raise HTTPException(status_code=403, detail='Not authorized')
